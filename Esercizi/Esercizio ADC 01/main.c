...
for(int i=0;i<100;i++) printf("%.2f V\n", v[i]);
float somma;
float SD;
for(int i=0;i<100;i++) somma+= v[i];
float media=somma/100;
for (int i = 0; i < 100; i++)
        SD += pow(v[i] - media, 2);
float deviazione= sqrt(SD / 99);

printf("la media è %.2f e la deviazione standrd è %.2f", media, deviazione); 
}//end main 
